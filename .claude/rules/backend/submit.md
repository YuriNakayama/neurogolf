...
## 提出時 validation の hard / soft 区別（重要）

`validate_onnx_files` は提出前に各 `taskNNN.onnx` を `audit_one` で検査するが、**失敗を hard と soft に分ける**:

- **hard（提出を中止）**: `FILESIZE_OVER_LIMIT` / `load_error` / `BANNED_OP` / `sanitize_failed` — 構造的に失格。`ValidationError` を送出。
- **soft（提出に含める）**: `score_error` / `session_error` — このプラットフォームの onnxruntime が**コスト推定のためにグラフを load できない**だけ（MaxPool/ConvTranspose の負パッド等）。Kaggle は正常採点するため **false-negative**。`TaskValidation(scorable=False, cost=None)` として残し、zip に含める。

→ **soft 失敗で提出全体を中止してはならない**。これを hard 扱いすると実 Kaggle win（7172.43 floor の t045/127/135/146/149/240/384 等）を取りこぼす。Kaggle が最終審判。

## 採否フロー（LB ゲート）

- ローカル scorer（`src/evaluate.audit_one`）は Kaggle 本番より弱く、上記 soft タスクで **false-negative** を出す。**ローカル単独 gating で override 採否を決めない**。
