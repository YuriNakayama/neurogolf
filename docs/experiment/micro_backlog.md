...
| 118 | residual table pruning after case352 | `36576 -> 35807` | `0.021249` | exact but high-risk, banked case360 | Scratch: `/private/tmp/neurogolf_task118_deep4/task118_pos_keep_first_neg_greedy_pos_any_greedy.onnx`. Local `n_fail=0`, functions=0, no forbidden ops. This table-pruning family is hidden-sensitive and below high-risk standalone threshold from current baseline; do not batch with low-risk cleanup. |
...
| 370 | direct first-valid selector plus sign aliases | `10393 -> 10351` | `0.004050` | exact micro, banked case369 | Scratch: `/tmp/task370_case369_probes/direct_plus_sign_pn_np_nn_col.onnx`. Repeat-branch deletion and mask aliases still fail correctness; keep only for a future low-risk/medium-risk batch. |
...
| 009 | bypass `Unsqueeze(content_3d_u8) -> content_u8` | `8410 -> 8310` | `0.011962` | exact micro, banked case381 | Scratch: `/tmp/neurogolf_case381_bundle/task009_content_unsq.onnx`. Single non-Clip structural reduction, below standalone submit gate. |
| 209 | `kc_for_min -> COL`, pruning sentinel `Where(col_haskey_b, COL, BIG_I)` before `ReduceMin` | `28047 -> 27927` | `0.004288` | exact micro, banked case381 | Scratch: `/tmp/neurogolf_case381_bundle/task209_kc_for_min_col.onnx`. Low-medium hidden risk if private cases need the no-key-column sentinel path; batch only. |
...
| 285 | `TopK` input bypass `color_idx_flat_f -> color_idx_flat` | `24535 -> 22702` | `0.077648` | exact but runtime-risk, banked case382 | Scratch: `/tmp/neurogolf_case382_main/task285_8_Cast_in0.onnx`. Feeds uint8 directly into `TopK`; treat like the task233 uint8 `TopK` Kaggle ERROR family unless runtime compatibility is proven. |
| 293 | bypass `Squeeze(col_valid_k) -> col_valid` | `1288 -> 1286` | `0.001554` | exact micro, banked case382 | Scratch: `/tmp/neurogolf_case382_main/task293_29_Squeeze_in0.onnx`. Too small alone. |
| 308 | bypass `And(..., lt_full) -> diamond` | `1488 -> 1484` | `0.002692` | exact micro, banked case382 | Scratch: `/tmp/neurogolf_case382_main/task308_38_And_in1.onnx`. Too small alone. |
